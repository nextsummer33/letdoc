...
async function mdToHtml(mdContent, template = default_template, theme = default_theme, metadata = {
  title: 'Project title',
  author: 'Your name',
  version: 'v1.0'
}) {
  ...
  templateData = templateData.replace(/{{doc_title}}/g, metadata.title)
  templateData = templateData.replace(/{{doc_version}}/g, metadata.version)
  templateData = templateData.replace(/{{doc_author}}/g, metadata.author)
  templateData = templateData.replace(/{{doc_date}}/g, (new Date).toLocaleDateString())
  templateData = templateData.replace(/{{doc_style}}/g, themeData)
  templateData = templateData.replace(/{{doc_content}}/g, converter.makeHtml(mdContent))
